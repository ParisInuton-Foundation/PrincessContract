...
contract ParisInuton is IBEP20, Authen, SafeToken {
    using SafeMath for uint256;
    ...
    string constant _name = "Paris Inuton";
    string constant _symbol = "PRINCESS";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000 * 10 ** 9 * (10 ** _decimals); // 1000 billion
    ...
    uint256 totalBuyFee = liquidityFee.add(buybackFee).add(reflectionFee).add(marketingFee);
    ...
    function includeToWhiteList(address _user) external onlyOwner {
    ...
    function setSafeManager(address _safeManager) external onlyOwner override{
